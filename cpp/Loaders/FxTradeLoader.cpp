...
// NOTE: These methods are only here to allow the solution to compile prior to
...
std::vector<std::string> split(const std::string &s, const std::string &sep) {
  ...
  while ((end = s.find(sep, start)) != std::string::npos) {
    ...
    // use the delmiter's length account for variable length delimiter, (this
    // would be ut8-characters)
...
FxTrade *FxTradeLoader::createTradeFromLine(std::string line) {
  ...
  auto id = items[8];
  FxTrade *trade =
      fxType.has_value() ? new FxTrade(id, fxType.value()) : new FxTrade(id);
...
void FxTradeLoader::loadTradesFromFile(std::string filename,
                                       FxTradeList &tradeList) {
  if (filename.empty()) {
