...
public:
  ...
  public:
    ...
    Iterator(std::vector<std::string>::const_iterator iter,
             const ScalarResults *parent_class)
        : iter_(iter), parent_(parent_class) {}
  ...
  Iterator begin() const {
    ...
    for (auto &[tradeId, _] : results_)
      allcurrentIds_.push_back(tradeId);
    for (auto &[id, _] : errors_) {
      if (std::find(allcurrentIds_.begin(), allcurrentIds_.end(), id) ==
          allcurrentIds_.end()) {
  ...
  Iterator end() const { return Iterator(allcurrentIds_.cend(), this); }
